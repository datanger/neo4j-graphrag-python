...
class MatlabPostProcessor:
    ...
    def _process_variable_assignments(self, graph: Neo4jGraph):
        """处理变量间的ASSIGNED_TO关系 - 只在跨作用域变量使用时创建，避免重复"""
        # 预编译正则表达式
                ...
                for rhs_var in meaningful_rhs_vars:
                    ...
                    if self._is_cross_scope_variable(rhs_var, node, cross_scope_uses):
                        ...
                        if assignment_key not in created_assignments:
                            ...
                            # 确保是跨作用域的关系
                            if not self._same_scope(target_node.id, source_node.id):
                                self._add_relationship(
                                    graph, target_node.id, source_node.id, 'ASSIGNED_TO',
                                    {'source': 'cross_scope_assignment'}
                                )
                ...
                if func_name in self.func_name_to_nodes:
                    ...
                    if (node.label == 'Function' and 
                        ...
                        self._add_relationship(
                            ...
                            {'call_type': 'function_call', 'scope': 'cross_scope'}
                        )
                ...
                for match in pattern.finditer(content):
                    ...
                    if called_script in self.script_name_to_nodes:
                        ...
                        self._add_relationship(
                            ...
                            {'call_type': 'script_call', 'scope': 'cross_scope'}
                        )
    ...
    def _add_cross_scope_uses_relationship(self, graph: Neo4jGraph, caller_id: str, var_name: str, 
        ...
        if source_script == "entry_script":
            target_id = f"var_{var_name}_entry_script"
        else:
            target_id = f"var_{var_name}_{source_script}_{source_script}.m"
        ...
        if target_id not in self.node_by_id:
            ...
            var_node = Neo4jNode(
                ...
                properties={
                    'name': var_name,
                    'type': 'local',
                    'scope_id': f'{source_script}_{source_script}.m' if source_script != "entry_script" else 'entry_script',
                    'scope_type': 'script'
                }
            )
        ...
        self._add_relationship(
            ...
            {'access_type': 'cross_scope_variable', 'source_script': source_script}
        )
