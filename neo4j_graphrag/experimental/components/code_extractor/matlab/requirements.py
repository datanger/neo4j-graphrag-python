...
REQUIREMENTS = """
1. 作用域的定义：一个脚本和函数内就是一个独立的作用域，这是为了隔离其他作用域的变量，避免变量污染。
2. 每个作用域内的变量都单独生成节点，即使变量名相同。
4. 作用域隔离：不同脚本中的同名变量被视为不同的节点，不同函数中的同名变量被视为不同的节点。
5. 跨作用域变量访问：只有在执行顺序上先定义的变量才能被后续脚本访问。此时应当生成跨作用域的USES关系，该作用域内被USES的这个变量与跨作用域的同名变量间应当同时有ASSIGNED_TO关系，以此来体现跨作用域的变量依赖关系。
6. 每个变量不能独立存在，应当与作用域绑定，要么是脚本中的变量，要么是函数中的变量。
7. 每个函数也不能独立存在，函数的定义肯定是在脚本中，因此函数节点应当与脚本节点绑定。
"""
