...
async def test_cross_scope_assignment():
    """测试跨作用域ASSIGNED_TO关系生成功能"""
    print("=== 测试跨作用域ASSIGNED_TO关系生成功能 ===")
    
    # 创建测试目录和文件
    test_dir = Path("test_cross_scope_assignment")
...
"""
    ...
    # 测试：使用启动脚本
    print("\n--- 测试跨作用域ASSIGNED_TO关系生成 ---")
    MatlabExtractor.reset_global_registry()
    
    llm = MockLLM()
    extractor = MatlabExtractor(llm=llm, entry_script_path=str(entry_script_path))
    ...
    if final_result:
        print(f"使用启动脚本的结果: {len(final_result.graph.nodes)} 节点, {len(final_result.graph.relationships)} 关系")
...
if __name__ == "__main__":
    asyncio.run(test_cross_scope_assignment()) 